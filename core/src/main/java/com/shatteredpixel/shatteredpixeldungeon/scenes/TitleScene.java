...
public class TitleScene extends PixelScene {
	...
	public void create() {
		...
		StyledButton btnPlay = new StyledButton(GREY_TR, Messages.get(this, "enter")){
			...
			protected void onClick() {
				if (GamesInProgress.checkAll().size() == 0){
					...
					ShatteredPixelDungeon.switchScene(HeroSelectScene.class); //英雄选择场景
				} else {
		...
		StyledButton seedAbout = new StyledButton(Chrome.Type.GREY_BUTTON_TR, "选择种子"){
			@Override
			protected void onClick() {
				ShatteredPixelDungeon.switchScene( SeedScene.class );
			}
		};
		// seedAbout.icon(Icons.get(Icons.SHPX));
		add(seedAbout);
		...
		if (landscape()) {
			...
			seedAbout.setRect(btnNews.left(), btnAbout.bottom()+GAP, btnRankings.width(), BTN_HEIGHT);
		} else {
			...
			seedAbout.setRect(btnNews.left(), btnAbout.bottom()+GAP, btnPlay.width(), BTN_HEIGHT);
		}
